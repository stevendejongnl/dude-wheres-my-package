...
async def test_get_vapid_public_key(push_client: AsyncClient):
    response = await push_client.get("/api/v1/push/vapid-public-key")
    assert response.status_code == 200
    assert response.json()["publicKey"] == "fake-pub"


async def test_push_subscribe(push_client: AsyncClient):
    payload = {
...
async def test_push_subscribe_upserts_on_same_endpoint(push_client: AsyncClient, repo: PackageRepository):
    payload = {
        "endpoint": "https://push.example.com/v1/test",
        "p256dh": "key1",
        "auth": "auth1",
    ...
    await push_client.post("/api/v1/push/subscribe", json={**payload, "p256dh": "key2"})
    subs = await repo.get_all_push_subscriptions()
    assert len(subs) == 1
    assert subs[0]["p256dh"] == "key2"


async def test_push_unsubscribe(push_client: AsyncClient):
    payload = {
        "endpoint": "https://push.example.com/v1/test",
        "p256dh": "dGVzdGtleQ==",
        "auth": "dGVzdGF1dGg=",
